...
OGVFrameDecoder.prototype.decodeFrameHeader = function()
{
    // 1
    ...
    if(isPacket == 1)
    {
        throw new Error("OGVFrameDecoder:decodeFrameHeader: invalid packet");
    }
...
// 5
    ...
    {
        throw new Error("OGVFrameDecoder:decodeFrameHeader: (7.1.7) This frame is not decodable according to this specification.");
    }
 ...
 * @returns {string}
 */
OGVFrameDecoder.prototype.decodeVariableRunBitString = function(nbits,isLong)
{
    // 1
    var len = 0;
    ...
    while(true){
        ...
        while(!rstart)
        {
            ...
            if(isLong)
            {
                huff_info = OGVFrameDecoder.getLongRunHuffmanBits(huff_code);
            }
            else
            {
                huff_info = OGVFrameDecoder.getShortRunHuffmanBits(huff_code);
            }
        ...
        if(len > nbits)
        {
            throw new Error("OGVFrameDecoder.decodeLongRunBitString: 7.2.1.10 len > nbits ");
        }
 ...
 */
OGVFrameDecoder.getLongRunHuffmanBits = function(code)
{
    if(code.toString(2).length > 6)
    {
        throw new Error("OGVFrameDecoder.getLongRunHuffmanBits: code longer than 6 bits");
    }
 ...
 * @returns {*} a container holding rbits and rstart
 */
OGVFrameDecoder.getShortRunHuffmanBits = function(code)
{
    if(code.toString(2).length > 5)
    {
        throw new Error("OGVFrameDecoder.getShortRunHuffmanBits: code longer than 5 bits");
    }
 ...
 *
 */
OGVFrameDecoder.prototype.decodeCodedBlockFlags = function()
{
